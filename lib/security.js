...
exports.makePasswordHashed = (userId, plainPassword) =>
    new Promise(async (resolve, reject) => {
        ...
        try {
            const [rowUser, fieldUser] = await conn.execute('SELECT pwdSalt FROM user WHERE id = ?', [userId]);
...
exports.makeResidentNumHashed = (userId, plainResidentNum) =>
    new Promise(async (resolve, reject) => {
        // userId인자로 해당 유저 salt를 가져오는 부분
        try {
            const [rowUser, fieldUser] = await conn.execute('SELECT resSalt FROM user WHERE id = ?', [userId]);
            const resSalt = rowUser[0].resSalt;
            // 위에서 가져온 salt와 plainPassword를 다시 해시 암호화 시킴. (비교하기 위해)
            crypto.pbkdf2(plainPassword, resSalt, 9999, 64, 'sha512', (err, key) => {
                    if (err) reject(err);
                    resolve(key.toString('base64'));
            });
        } catch (err) {
